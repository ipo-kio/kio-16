...
public class ClockSprite extends Sprite {
    ...
    private var resultSprite:Sprite = new ResultSprite();


    
    public function ClockSprite(level:int) {
        _level = level;
        instanse=this;
        resultSprite.x = 115;
        ...
        transmissionMechanism.viewSide.x = 5;
    ...
    public function showError(text:String){
  ...
  private var _stopButton:ShellButton;
    ...
    private function addSettingsPanel():void {
        ...
        settingsPanel.graphics.drawRect(0, 0, 110, 400);
      ...
      _cbSpeed.addEventListener(Event.CHANGE, speedSelected);
        ...
        createButton("step", 5, 55, function (event:MouseEvent) {
            transmissionMechanism.rotate(SettingsHolder.instance.stepRotateInRadians);
        }, false);

        _playButton = createButton("play", 5, 85, function (event:MouseEvent) {
            transmissionMechanism.playStop();
        }, false);

        _stopButton = createButton("stop", 5, 85, function (event:MouseEvent) {
            transmissionMechanism.playStop();
        }, false);
        ...
        createButton("reset", 5, 115, function (event:MouseEvent) {
        ...
        _showButton = createButton("show", 5, 175, function (event:MouseEvent) {
        ...
        _hideButton = createButton("hide", 5, 175, function (event:MouseEvent) {
        ...
        createButton("delete", 5, 220, function (event:MouseEvent) {
        ...
        addSettingField("result", 0, 260, 0, function (e:KeyboardEvent):void {}, false);

        _numberField = addSettingField("transmission", 0, 280, 0, function (e:KeyboardEvent):void {
        });
        _squareField = addSettingField("square", 0, 320, 0, function (e:KeyboardEvent):void {
        });
...
//        });
    ...
    private function addSettingField(name:String, x:int, y:int, initialValue:int, action:Function, input=true):TextField {
        var loc:Object = KioApi.getLocalization(ClockProblem.ID);
    ...
    public function get etalon():Number {
        return (_level==0)?24:37;
    }                            
}
}
