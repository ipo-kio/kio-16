...
//            }
        ...
        for (Map.Entry<String, List<Integer>> entry : login2levels.entrySet()) {
            ...
            if (levels.size() >= 2) {
                ...
                manyLevelsCount++;
                totalLevelsCount += levels.size();
                if (filteredOutTimes.getOrDefault(login, 0) == levels.size())
                    System.out.println("Aaaaa, stupid participant " + login);
            }
        }
        System.out.println("total: " + manyLevelsCount + " (" + totalLevelsCount + ")");
    }

    private static void loadLevels() {
        try (Scanner in = new Scanner(new File("level-only-many.csv"))) {
//            in.nextLine();
    ...
    //all-solutions-2016/97655921.72.kio-0
    private static void processSolutionFile(File solution, String login, int level, int year, File[] solutions) {
        try {
            ...
            if (file.getYear() != 2016) {
                System.out.println("Wrong year in file " + solution);
                return;
            }
            ...
            //comment out: checking old files
    ...
    private static Map<String, Integer> filteredOutTimes = new HashMap<>();

    private static void filterUsersWithWrongLevel(int level) {
        Map<String, UserResults> newResults = new HashMap<>();
        for (Map.Entry<String, UserResults> e : results.entrySet()) {
            ...
            if (forcedLevel == null || forcedLevel == level)
                ...
                System.out.println("Filtered out user " + user + " with level " + forcedLevel + " instead of " + level);
                filteredOutTimes.put(user, filteredOutTimes.getOrDefault(user, 0) + 1);
            }
        }
